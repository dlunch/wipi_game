...
pub(super) fn as_u32(value: i32) -> u32 {
    value.max(0) as u32
}
...
pub(super) fn timed_to_status(timed: &TimedState) -> StatusRender {
    StatusRender {
...
pub(super) fn build_explore_render(
    world: &WorldState,
    ui: &UiState,
    data: &Rc<GameData>,
...
pub(super) fn build_tracked_quest_render(
    world: &WorldState,
    data: &Rc<GameData>,
...
pub(super) fn build_interaction_hint(world: &WorldState, data: &Rc<GameData>) -> Option<String> {
    let leader = world.leader_entity()?;
    let map = data.find_map(&leader.map_id)?;
...
pub(super) fn build_inventory_render(
    world: &WorldState,
    ui: &UiState,
    data: &Rc<GameData>,
) -> InventoryRender {
...
pub(super) fn build_stats_render(world: &WorldState) -> Option<StatsRender> {
    let leader_id = world.leader_id()?;
    let leader = world.leader_entity()?;
...
pub(super) fn build_shop_render(
    world: &WorldState,
    ui: &UiState,
    data: &Rc<GameData>,
    render_fx: &RenderFxState,
...
pub(super) fn build_quest_log_render(
    world: &WorldState,
    ui: &UiState,
    data: &Rc<GameData>,
) -> QuestLogRender {
